...
import com.sagor.util.EmailConstraint;
import com.sagor.util.EmailStatus;
...
public class MailService {
	...
	private EmailStatus sendMimeMail(String[] to, String subject, String body, Boolean isHtml, List<File> attachment) {
		...
		try {
			Session session = emailConstraint.getSessonInstance();// ekta mail send korte gele Session create korte hoy
			...
			logger.error("Error in sending mail" + to[0] + " message" + e.getMessage());
